...
每条陈述必须区分 author_statement、system_summary、system_inference；system_inference 必须 inferred=true，其他来源必须 inferred=false。Insight 不得只由 abstract_only=true 的证据支撑。每个 Ablation 必须引用 figure/table evidence。每个 SupportingVisual 必须解释该 Figure/Table 如何支撑对应 Insight。

输出必须严格符合提供的 JSON Schema。不要输出 Markdown 代码围栏、解释、前言或结尾。"""
